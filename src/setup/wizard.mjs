 ...
 * status 0. Measured 2026-08-28 on the shipped 1.1.0 build: `ledgerline setup`
...
export async function runSetup(opts = {}) {
  ...
  try {
    ...
    if (interactive) {
      ...
      say();
      say("  Skip all of it now and run `ledgerline provider` whenever you like.");
      say();
      ...
      say("  charged twice. Run `ledgerline provider signin <name>` when ready.");
      if (!enabled.length) {
        say("\n  Nothing added. The few keyless models that still answer will be used;");
        say("  run `ledgerline provider` whenever you want the rest.");
      }
    }
    ...
    if (!opts.skipBrowser) {
      ...
      if (chromiumInstalled()) {
        ...
        if (!r.ok) {
          ...
          say("  Browser tasks will not work. Re-run: ledgerline setup --browser");
    ...
    // terminal, and this block used to tell them to type `ledgerline`. Say
    // the thing they can actually do; keep the commands for a source checkout.
    const exe = pkg("..", "Ledgerline.exe");
    if (fs.existsSync(exe)) {
      say("  Open Ledgerline from your Desktop or Start Menu.");
      say(`  (or double-click ${exe})`);
    } else {
      say("  Open the app with:      ledgerline ui");
      say("  Check quota and usage:  ledgerline usage");
      say("  Re-run these checks:    ledgerline doctor");
    }
    say();
