...
export function permissionProfile(name) {
  ...
  //
  // 🔴 NOT the whole data directory. `%LOCALAPPDATA%\Ledgerline` holds the
  ...
  if (permission.external_directory !== "allow") {
    permission.external_directory = {
      ...
      // ...\Local\Ledgerline\downloads\... while OpenCode sees
      // ...\Local\Packages\<pkg>\LocalCache\Local\Ledgerline\downloads\..., so
      // the absolute patterns above do not match. These are scoped to the WORK
      // directories only - the old "**/Ledgerline/**" net covered the secrets
      // too, which is exactly the hole above.
      "**/Ledgerline Workspace/**": "allow",
      "**/Ledgerline/downloads/**": "allow",
    };
  }
  return permission;
}
 ...
 * The Ledgerline plugin is written as a one-line re-export shim pointing at the
...
function installPlugins() {
  ...
  fs.writeFileSync(
    path.join(pluginsDir, "ledgerline.js"),
    `// Generated by ledgerline setup. Re-exports the installed plugin.\n` +
      `export { LedgerlinePlugin } from ${JSON.stringify(realPlugin)};\n`
  );
...
export function writeOpenCodeConfig(opts = {}) {
  ...
  const plugins = ["./plugins/ledgerline.js"];
  if (omniroutePlugin) {
    plugins.push([
      omniroutePlugin,
      {
        providerId: "omniroute",
        displayName: "Ledgerline Models",
        baseURL,
  ...
  const config = {
    ...
    agent: {
      omni: {
        ...
        mode: "primary",
        prompt:
          "You are Ledgerline. You can write and run code, browse the real web with a real browser, " +
